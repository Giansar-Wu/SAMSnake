...
```
conda create -n samsanke python=3.11
conda activate samsnake

# install pytorch, the cuda version is 12.1
...
1. Download the COCO 2017 dataset at [here](https://cocodataset.org/#download).
...
1. Download the COCO 2014 dataset at [here](https://cocodataset.org/#download).

2. Organize the dataset as the following structure:

   ```
   ├── /path/to/coco
   │   ├── COCO_amodal_val2014_with_classes_amodal.json
   │   ├── COCO_amodal_train2014_with_classes_amodal.json
   │   ├── train2014
   │   ├── val2014
   │   ├── test
   ```
