...
def polygon_to_cmask(poly, h, w):
    ...
    # try:
    #     print(type(poly), len(poly), poly[0][0].shape, poly[0][0].dtype)
    # except:
    #     print('xxxx', type(poly), len(poly), type(poly[0]), len(poly[0]))
    # print(len(poly), type(poly[0]))
    # if len(poly) > 1:
    polyn = []
    for i in poly:
        if len(i):
...
def augment(img,
    ...
    Args:
        img (_type_): cv2 img
        split (_type_): 'train' 'val' or 'test'
        _data_rng (_type_): RandomState random seed
        _eig_val (_type_): np.array
        _eig_vec (_type_): np.array
        mean (_type_): _description_
        std (_type_): _description_
        down_ratio (_type_): _description_
        input_h (_type_): _description_
        input_w (_type_): _description_
        scale_range (_type_): _description_
        scale (_type_, optional): _description_. Defaults to None.
        test_rescale (_type_, optional): _description_. Defaults to None.
        test_scale (_type_, optional): _description_. Defaults to None.
...
def four_idx(img_gt_poly):
    ...
    # 变相的归一化？？偏移量的归一化？？
    # 只有从中心出发，上下左右四个方向can_gt_polys的值为1或-1
    can_gt_polys /= np.repeat(distance, axis=1, repeats=2)
    idx_bottom = np.argmax(can_gt_polys[:, 1]) #下方点 索引为一个单值
    idx_top = np.argmin(can_gt_polys[:, 1]) #上方点
    idx_right = np.argmax(can_gt_polys[:, 0]) #右方点
    idx_left = np.argmin(can_gt_polys[:, 0]) #左方点
    return [idx_bottom, idx_right, idx_top, idx_left] # 下 右 上 左

def get_img_gt(img_gt_poly, idx, t=128):
    align = len(idx)
    pointsNum = img_gt_poly.shape[0] # 点的总数
    r = []
...
def img_poly_to_can_poly(img_poly):
    ...
    x_max, y_max = np.max(img_poly, axis=0)
    can_poly = img_poly - np.array([x_min, y_min])
    # can_poly = (img_poly - np.array([x_min, y_min])) / np.array([x_max - x_min, y_max - y_min])
    return can_poly
