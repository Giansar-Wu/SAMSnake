...
def make_ddp_train_loader(cfg, gpus: int):
    batch_size = int(cfg.train.batch_size / gpus)
    shuffle = True
    drop_last = False
    dataset_name = cfg.train.dataset
...
def make_ddp_data_loader(cfg, gpus: int, train=True, is_distributed=False):
    if train:
        # return make_ddp_train_loader(cfg, gpus), make_test_loader(cfg, is_distributed)
        return make_ddp_train_loader(cfg, gpus)
    else:
        return make_test_loader(cfg, is_distributed)
        # return make_ddp_test_loader(cfg)
