...
class Evolution(nn.Module):
    def __init__(self, evole_ietr_num=3, evolve_stride=1., ro=4., num_points = 128, use_normal: bool=True):
    ...
    def evolve_poly(self, 
                    ...
                    i_it_poly:torch.Tensor, 
                    c_it_poly:torch.Tensor, 
                    ind, 
                    stride=1., 
                    ignore=False):
        """_summary_
        ...
        Args:
            ...
            cnn_feature (_type_): (N,C,H,W)
            i_it_poly (_type_): (batch_ins_num, num_points_, 2)
            c_it_poly (_type_): (batch_ins_num, num_points_, 2)
            ind (_type_): (batch_ins_num,)
            stride (_type_, optional): _description_. Defaults to 1..
            ignore (bool, optional): _description_. Defaults to False.
        ...
        if self.use_normal:
            ...
            offset = torch.tanh(snake(init_input).permute(0, 2, 1))
            w_h = i_it_poly.max(dim=1)[0] - i_it_poly.min(dim=1)[0] # (batch_ins_num, 2)
            i_poly = i_it_poly * self.ro + offset * (w_h[:,None,:].repeat((1, self.num_points, 1))) * self.ro * stride
        else:
    ...
    def foward_train(self, output, batch, cnn_feature):
        ret = output
        # img_gt_polys 相对于output_size的真值坐标
        # img_init_polys 相对于output_size的预测值坐标
        # can_init_polys 相对于output_size的预测值归一化坐标
        # py_ind，每个中心点属于哪个图片
        init = self.prepare_training(output, batch)
        py_pred = self.evolve_poly(self.evolve_gcn, cnn_feature, init['img_init_polys'], init['can_init_polys'], init['py_ind'], stride=self.evolve_stride)
        py_preds = [py_pred]
        for i in range(self.iter):
            py_pred = py_pred / self.ro
            c_py_pred = img_poly_to_can_poly(py_pred)
            evolve_gcn = self.__getattr__('evolve_gcn' + str(i))
            py_pred = self.evolve_poly(evolve_gcn, cnn_feature, py_pred, c_py_pred, init['py_ind'], stride=self.evolve_stride)
            py_preds.append(py_pred)
        ret.update({'py_pred': py_preds, 'img_gt_polys': init['img_gt_polys'] * self.ro})
        
        # 输出相对于input_size的预测值与真值
        return output
    ...
    def foward_test(self, output, cnn_feature, ignore):
        ...
        with torch.no_grad():
            ...
            py = self.evolve_poly(self.evolve_gcn, cnn_feature, img_init_polys, init['can_init_polys'], init['py_ind'], stride=self.evolve_stride, ignore=ignore[0])
            pys = [py, ]
            for i in range(self.iter):
                py = py / self.ro
