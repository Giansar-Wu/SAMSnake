...
def collect_training(poly, ct_01: torch.Tensor):
    ct_01 =ct_01.type(torch.bool)
...
def prepare_training(ret, batch, ro):
    ...
    init.update({'img_gt_polys': collect_training(batch['img_gt_polys'], ct_01)}) # 相对于output_size的真值坐标
    init.update({'img_init_polys': ret['poly_coarse'].detach() / ro}) # 相对于output_size的预测值坐标
    can_init_polys = img_poly_to_can_poly(ret['poly_coarse'].detach() / ro) # 相对于output_size的预测值归一化坐标
    init.update({'can_init_polys': can_init_polys})

    ct_num = batch['meta']['ct_num']
    init.update({'py_ind': torch.cat([torch.full([ct_num[i]], i) for i in range(ct_01.size(0))], dim=0)}) # [0，1，2，2，3，4，4，4] 表示第一个中心点属于图片0，第二个属于图片1，第三四个属于图片2
    init.update({'py_ind': init['py_ind']})
    init['py_ind'] = init['py_ind'].to(ct_01.device)

    return init

def img_poly_to_can_poly(img_poly, normal: bool=True):
    if len(img_poly) == 0:
...
def get_gcn_feature(cnn_feature, img_poly, ind, h, w):
    ...
    for i in range(batch_size):
        ...
        feature = torch.nn.functional.grid_sample(cnn_feature[i:i+1], poly, align_corners=False)[0].permute(1, 0, 2)
