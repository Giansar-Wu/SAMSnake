...
class commen(object):
    ...
    @classmethod
    def init_dir(cls):
        t = time.strftime('%Y%m%d_%H_%M', time.localtime())
        os.makedirs(os.path.join("runs", cls.dataset, t, "weights"), exist_ok=True)
        cls.record_dir = os.path.join("runs", cls.dataset, t)
        cls.result_dir = os.path.join("runs", cls.dataset, t)
        cls.model_dir = os.path.join("runs", cls.dataset, t, "weights")
...
class train(object):
    save_ep = 5
    eval_ep = 5
    optimizer = {'name': 'adam', 'lr': 1e-4, 'weight_decay': 5e-4}
    scheduler = {'name': 'MultiStepLR', 'milestones': [80, 120, ], 'gamma': 0.5}
    batch_size = 24
    num_workers = 8
    epoch = 150
    with_dml = True
